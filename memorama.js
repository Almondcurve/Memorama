...
function play() {
	const respuesta = readlineSync.question('Ingrese dos cartas para revelar: ')

	const [card1, card2] = respuesta.split(' ').map(Number);

	if (!Number.isNaN(card1) && !Number.isNaN(card2) && card1 !== card2 && card1 >= 0 && card2 >= 0 && card1 < shuffledCards.length &&
		card2 < shuffledCards.length && !revealedCards.includes(card1) && !revealedCards.includes(card2)) {
